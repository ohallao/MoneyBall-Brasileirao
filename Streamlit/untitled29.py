...
def radar_chart_por_jogadores(jogadores_selecionados, df, metrics):
    ...
    for _, row in jogadores_filtrados.iterrows():
        valores = row[metrics].tolist()
        valores += valores[:1]  # Fechar o círculo
        ax.plot(angles, valores, label=row['jogador'], linestyle='solid', linewidth=2)
        ax.fill(angles, valores, alpha=0.25)
    ...
    plt.title("Comparação de Jogadores Selecionados", size=16, color='black', y=1.1)
...
if uploaded_file is not None:
    ...
    if posicao_escolhida in pesos_por_posicao:
        ...
        ranking = df.sort_values(by="Pontuacao_Total", ascending=False).reset_index()
