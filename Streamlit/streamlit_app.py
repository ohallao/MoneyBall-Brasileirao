...
if uploaded_file is not None:
    ...
    st.write(striker.head(10))  # Limitando a visualização a 10 linhas
    ...
    resultado_final = jogadores_ordenados[
        ['Ranking', 'time', 'posicao', 'jogador', 'Perfil']  # Colunas pontuação removidas
    ]
    ...
    jogadores_selecionados = st.multiselect(
        "Selecione os jogadores para comparar:", striker['jogador'].unique()  # Seleção de qualquer cluster
    )

    if len(jogadores_selecionados) > 0:
        radar_chart_por_jogadores(jogadores_selecionados, striker)

        # Opção para seleção de colunas da tabela
        colunas_disponiveis = [col for col in jogadores_filtrados.columns if col not in (
            'PrgR_Pontuacao', 'xG_Pontuacao', 'xAG_Pontuacao', 'G-PK_Pontuacao',
            'Assistencias_Pontuacao', 'Acoes Ofensivas_Pontuacao', 'Aerial Duel%_Pontuacao',
            'Pontuacao_Total', 'Pontuacao'
        )]

        colunas_selecionadas = st.multiselect(
            "Selecione as colunas para exibir na tabela:", colunas_disponiveis, default=colunas_disponiveis
        )

        # Tabela com colunas selecionadas
        tabela_metricas = striker[striker['jogador'].isin(jogadores_selecionados)][colunas_selecionadas]
        st.write("### Estatísticas dos Jogadores Selecionados")
        st.dataframe(tabela_metricas)

    # Opção para editar o nome do perfil
    st.sidebar.header("Editar Nome do Perfil")
    novo_nome_perfil = st.sidebar.text_input("Novo nome para o perfil selecionado:")
    if st.sidebar.button("Atualizar Nome do Perfil"):
        if novo_nome_perfil:
            striker.loc[striker['Perfil'] == perfil_escolhido, 'Perfil'] = novo_nome_perfil
            st.sidebar.success(f"Nome do perfil atualizado para: {novo_nome_perfil}")
else:
